...
const Solutions = () => {
  ...
  const SolutionCard = ({ solution, featured = false }) => (
    <div
      className={`group relative overflow-hidden rounded-[30px] border border-white/10 bg-slate-900/70 shadow-[0_20px_60px_-30px_rgba(0,0,0,0.35)] backdrop-blur-sm transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl ${
        featured ? 'lg:col-span-2' : ''
      }`}
    >
      <div
        className={`pointer-events-none absolute inset-0 bg-gradient-to-br ${solution.gradient} opacity-0 transition-opacity duration-500 group-hover:opacity-10`}
      />

      <div className="absolute left-4 top-4 z-20">
        <span className="inline-flex items-center rounded-full bg-slate-800/80 px-3 py-1 text-xs font-medium text-slate-200 shadow-sm backdrop-blur-sm">
          {solution.category}
        </span>
      </div>
      ...
      <div className={`relative overflow-hidden ${featured ? 'h-72' : 'h-60'}`}>
        ...
        <div
          className={`absolute bottom-4 left-4 z-20 flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br ${solution.gradient} text-white shadow-xl`}
        >
          {solution.icon}
        </div>
      </div>
      ...
      <div className="relative z-20 p-6">
        ...
        <Link
          to={solution.link}
          className={`inline-flex items-center rounded-2xl bg-gradient-to-r ${solution.gradient} px-6 py-3 text-sm font-semibold text-white shadow-lg transition-all duration-300 hover:scale-105 hover:shadow-xl`}
        >
  ...
  return (
    <section className="relative overflow-hidden bg-transparent py-20 text-white md:py-24 lg:py-28">
